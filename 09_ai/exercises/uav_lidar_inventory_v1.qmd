...
```{r eval = FALSE}
example_tree <- 1107 # select a tree ID of your choice

las_instance <- filter_poi(las, PredInstance == randomtree)
plot(las_instance, color = "Z")
```
...
xmin <- 416600
xmax <- 416900  # Narrow strip along X
ymin <- 5346790
ymax <- 5346800
...
inner_join(centroids, tree_heights2, by = "PredInstance")
...
### 3) Calculate DBH via allometries (only when we know the species of the datasets)
...
### 5 Compare stand-level metrics from a drone-based inventory and ground-based inventory
